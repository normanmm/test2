...
const customSelect = function (element, overrides){
	...
	//Norman 6.12.22 const csIcons = csSelector.querySelectorAll('svg')
	...
	/* Norman 7.12.22 - deactivate Aria implementation
	...
	// Norman 15.12.22 import/load contents of houses.json file into const object houses
	let houses={}; //global object
	//let x=0;
	async function importHouses() {

		const requestURL = 'houses.json';
		...
		//const strHouses=JSON.stringify(houses);
		//return(houses [0] ['lat']);		
		//alert(houses [0] ['lat']);
		//alert(JSON.stringify(houses));
		//	alert("Hi");
		alert(houses [0] ['lat']);
		//alert(JSON.stringify(Houses));
		//x=1;
	...
	function doFilter() {
		...
		const aFilteredOptions = aOptions.filter(function(option) {
			//Norman commented- if (option.innerText.toUpperCase().startsWith(terms.toUpperCase())) {
